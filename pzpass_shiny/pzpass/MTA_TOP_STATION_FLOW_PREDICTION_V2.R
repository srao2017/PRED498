...
#train_datafile<-"TRAINING/DailyExitsEntries-top9.csv"
#train_datafile<-"TRAINING/DailyExitsEntries-all.csv"

validation_datafile<-"VALIDATION/processed_30_20170722.csv"
#validation_datafile<-"VALIDATION/processed_top9_20170722.csv"
#validation_datafile<-"VALIDATION/processed_all_20170722.csv"
...
PlotScores<-function(model_type="gbm", subway="FULTON ST", day=3, week=26) {
  ...
  p<-ggplot(d, aes(Hour,value,col=variable)) + 
    ...
    theme(axis.text.y = element_text(size=15)) + theme(plot.title=element_text(margin = margin(t=30,b=-20)))+labs(title=toString(subway))
  p
}
#------------------------------END FUNCTIONS-------------------------------
...
set.seed(825)
gbmFit1 <- train(Ent~factor(Station) + Month + Week + Day + Hour, data = X, 
                 method = "gbm", 
...
boost.fitEnt<-gbm(Ent~Station + Month + Week + Day + Hour, data=X, distribution="gaussian", n.trees=150, interaction.depth=8, shrinkage=1)
...
boost.fitExt<-gbm(Ext~Station + Month + Week + Day + Hour, data=X, distribution="gaussian", n.trees=150, interaction.depth=8, shrinkage=1)
...
X<-Score(X,"gbm")
print(PlotScores("gbm",subway="59 ST COLUMBUS",day=5,week=30))

# Validate on test data
Z$PredEnt<-abs(predict(boost.fitEnt, n.trees=15000, newdata=Z))
#Compute test MSE
mse<-mean((Z$PredEnt - Z$Ent)^2)
se<-sqrt(mse)
se
#Z$EntMse<-Z$PredEnt - Z$Ent
Z$PredExt<-abs(predict(boost.fitExt, n.trees=15000, newdata=Z))
#Compute test MSE
...
Z<-Score(Z,"gbm")
print(PlotScores("gbm",subway="59 ST COLUMBUS",day=4))
